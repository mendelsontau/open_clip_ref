...
def multi_head_attention_forward(query: Tensor,
    ...
    Shape:
        ...
        - attn_output_weights: :math:`(N, L, S)` where N is the batch size,
          L is the target sequence length, S is the source sequence length.
    """
    #tgt_len = number of tokens
    #bsz = batch size
    #embed_dim = token length
...
def multi_head_attention_prompts_forward(query: Tensor,
                                 key: Tensor,
                                 value: Tensor,
                                 embed_dim_to_check: int,
                                 num_heads: int,
                                 ...
                                 in_proj_weight: Tensor,
                                 in_proj_bias: Tensor,
                                 in_prompts_proj_weight: Tensor,
                                 in_prompts_proj_bias: Tensor,
                                 in_proj_weight_A: Tensor,
                                 in_proj_weight_B: Tensor,
                                 in_proj_weight_scaling: Tensor,
                                 bias_k: Optional[Tensor],
                                 ...
                                 out_proj_weight: Tensor,
                                 out_proj_bias: Tensor,
                                 out_prompts_proj_weight: Tensor,
                                 out_prompts_proj_bias: Tensor,
                                 out_proj_weight_A: Tensor,
                                 out_proj_weight_B: Tensor,
                                 out_proj_weight_scaling: Tensor,
                                 training: bool = True,
    ...
    if not use_separate_proj_weight:
        if torch.equal(query, key) and torch.equal(key, value):
            # self-attention
            query_patch_cls = torch.cat([query[0,:,:].unsqueeze(dim=0),query[num_prompts+1:,:,:]])
            qkv = linear(query_patch_cls, in_proj_weight, in_proj_bias)
            qkv += linear(linear(query_patch_cls, in_proj_weight_A), in_proj_weight_B) * in_proj_weight_scaling
            query_prompts = query[1:num_prompts + 1,:,:]
            qkv_prompts = linear(query_prompts, in_prompts_proj_weight, in_prompts_proj_bias)
            q, k, v = qkv.chunk(3, dim=-1)
            q_prompts, k_prompts, v_prompts = qkv_prompts.chunk(3, dim=-1)
            q = torch.cat([q[0,:,:].unsqueeze(dim=0),q_prompts,q[1:,:,:]])
            k = torch.cat([k[0,:,:].unsqueeze(dim=0),k_prompts,k[1:,:,:]])
            v = torch.cat([v[0,:,:].unsqueeze(dim=0),v_prompts,v[1:,:,:]])
    ...
    assert list(attn_output.size()) == [bsz * num_heads, tgt_len, head_dim]
    attn_output_ = attn_output.transpose(0, 1).contiguous().view(tgt_len, bsz, embed_dim)
    attn_output_patch_cls = torch.cat([attn_output_[0,:,:].unsqueeze(dim=0),attn_output_[num_prompts+1:,:,:]])
    attn_output = linear(attn_output_patch_cls, out_proj_weight, out_proj_bias)
    attn_output += linear(linear(attn_output_patch_cls, out_proj_weight_A), out_proj_weight_B) * out_proj_weight_scaling
    attn_output_prompts = attn_output_[1:num_prompts + 1,:,:]
    attn_output_prompts = linear(attn_output_prompts, out_prompts_proj_weight, out_prompts_proj_bias)
    attn_output = torch.cat([attn_output[0,:,:].unsqueeze(dim=0),attn_output_prompts,attn_output[1:,:,:]])
...
def multi_head_attention_prompts_forward2(query: Tensor,
                                 key: Tensor,
                                 value: Tensor,
                                 embed_dim_to_check: int,
                                 num_heads: int,
                                 ...
                                 in_proj_weight_A: Tensor,
                                 in_proj_weight_B: Tensor,
                                 in_prompts_proj_weight_A: Tensor,
                                 in_prompts_proj_weight_B: Tensor,
                                 in_proj_weight_scaling: Tensor,
                                 bias_k: Optional[Tensor],
                                 bias_v: Optional[Tensor],
                                 add_zero_attn: bool,
                                 ...
                                 out_proj_weight_A: Tensor,
                                 out_proj_weight_B: Tensor,
                                 out_prompts_proj_weight_A: Tensor,
                                 out_prompts_proj_weight_B: Tensor,
                                 out_proj_weight_scaling: Tensor,
                                 training: bool = True,
                                 key_padding_mask: Optional[Tensor] = None,
                                 need_weights: bool = True,
    ...
    if not use_separate_proj_weight:
        if torch.equal(query, key) and torch.equal(key, value):
            ...
            query_prompts = query[1:num_prompts + 1,:,:]
            qkv_prompts = linear(query_prompts, in_prompts_proj_weight, in_prompts_proj_bias)
            qkv_prompts += linear(linear(query_prompts, in_prompts_proj_weight_A), in_prompts_proj_weight_B) * in_proj_weight_scaling
            q, k, v = qkv.chunk(3, dim=-1)
            q_prompts, k_prompts, v_prompts = qkv_prompts.chunk(3, dim=-1)
            q = torch.cat([q[0,:,:].unsqueeze(dim=0),q_prompts,q[1:,:,:]])
            k = torch.cat([k[0,:,:].unsqueeze(dim=0),k_prompts,k[1:,:,:]])
    ...
    attn_output = linear(attn_output_patch_cls, out_proj_weight, out_proj_bias)
    attn_output += linear(linear(attn_output_patch_cls, out_proj_weight_A), out_proj_weight_B) * out_proj_weight_scaling
    attn_output_prompts1 = attn_output_[1:num_prompts + 1,:,:]
    attn_output_prompts = linear(attn_output_prompts1, out_prompts_proj_weight, out_prompts_proj_bias)
    attn_output_prompts += linear(linear(attn_output_prompts1, out_prompts_proj_weight_A), out_prompts_proj_weight_B) * out_proj_weight_scaling
    attn_output = torch.cat([attn_output[0,:,:].unsqueeze(dim=0),attn_output_prompts,attn_output[1:,:,:]])
...
class MultiheadAttentionPrompts(nn.Module):
    r"""Allows the model to jointly attend to information
    from different representation subspaces.
    See reference: Attention Is All You Need
    .. math::
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False, kdim=None, vdim=None, lora_alpha: int = 1, r=0, num_prompts = 10, prompts_lora=-1):
        super(MultiheadAttentionPrompts, self).__init__()
        self.embed_dim = embed_dim
        self.kdim = kdim if kdim is not None else embed_dim
        self.vdim = vdim if vdim is not None else embed_dim
        self._qkv_same_embed_dim = self.kdim == embed_dim and self.vdim == embed_dim
        ...
        if self._qkv_same_embed_dim is False:
            ...
            if prompts_lora == -1:
                ...
                self.in_proj_weight_lora_A = nn.Parameter(torch.empty(r, embed_dim))
                self.in_proj_weight_lora_B = nn.Parameter(torch.empty(3 * embed_dim, r))
                self.in_prompts_proj_weight = nn.Parameter(torch.empty(3 * embed_dim, embed_dim))
                self.register_parameter('q_proj_weight', None)
                self.register_parameter('k_proj_weight', None)
                self.register_parameter('v_proj_weight', None)
                self.register_parameter('q_prompts_proj_weight', None)
                self.register_parameter('k_prompts_proj_weight', None)
                self.register_parameter('v_prompts_proj_weight', None)
                self.register_parameter('q_proj_weight_lora_A', None)
                self.register_parameter('k_proj_weight_lora_A', None)
                self.register_parameter('v_proj_weight_lora_A', None)
                self.register_parameter('q_proj_weight_lora_B', None)
                ...
                self.in_proj_weight_lora_B = nn.Parameter(torch.empty(3 * embed_dim, r))
                self.in_prompts_proj_weight = nn.Parameter(torch.empty(3 * embed_dim, embed_dim))
                self.in_prompts_proj_weight_lora_A = nn.Parameter(torch.empty(prompts_lora, embed_dim))
                self.in_prompts_proj_weight_lora_B = nn.Parameter(torch.empty(3 * embed_dim, prompts_lora))
                self.register_parameter('q_proj_weight', None)
                self.register_parameter('k_proj_weight', None)
                self.register_parameter('v_proj_weight', None)
                self.register_parameter('q_proj_weight_lora_A', None)
                ...
                self.register_parameter('k_prompts_proj_weight', None)
                self.register_parameter('v_prompts_proj_weight', None)
                self.register_parameter('q_prompts_proj_weight_lora_A', None)
                self.register_parameter('k_prompts_proj_weight_lora_A', None)
                self.register_parameter('v_prompts_proj_weight_lora_A', None)
                self.register_parameter('q_prompts_proj_weight_lora_B', None)
                self.register_parameter('k_prompts_proj_weight_lora_B', None)
                self.register_parameter('v_prompts_proj_weight_lora_B', None)


        if bias:
            self.in_proj_bias = nn.Parameter(torch.empty(3 * embed_dim))
            self.in_prompts_proj_bias = nn.Parameter(torch.empty(3 * embed_dim))
        else:
            self.register_parameter('in_proj_bias', None)
        self.out_proj = Linear(embed_dim, embed_dim, bias=True, merge_weights=False, lora_alpha=lora_alpha, r=r)
        if prompts_lora == -1:
            self.out_prompts_proj = nn.Linear(embed_dim, embed_dim, bias=True)
        else:
            self.out_prompts_proj = Linear(embed_dim, embed_dim, bias=True, merge_weights=False, lora_alpha=lora_alpha, r=prompts_lora)
    ...
    def _reset_parameters(self):
        if self._qkv_same_embed_dim:
            if self.prompts_lora == -1:
                xavier_uniform_(self.in_proj_weight)
                xavier_uniform_(self.in_prompts_proj_weight)
                xavier_uniform_(self.in_proj_weight_lora_A)
                xavier_uniform_(self.in_proj_weight_lora_B)
            else:
                xavier_uniform_(self.in_proj_weight)
                ...
                xavier_uniform_(self.in_proj_weight_lora_B) 
                xavier_uniform_(self.in_prompts_proj_weight)
                xavier_uniform_(self.in_prompts_proj_weight_lora_A)
                xavier_uniform_(self.in_prompts_proj_weight_lora_B)                  
        else:
            xavier_uniform_(self.q_proj_weight)
            xavier_uniform_(self.k_proj_weight)
            xavier_uniform_(self.v_proj_weight)
        ...
        if self.in_proj_bias is not None:
            constant_(self.in_proj_bias, 0.)
            constant_(self.out_proj.bias, 0.)
            constant_(self.in_prompts_proj_bias, 0.)
            constant_(self.out_prompts_proj.bias, 0.)
        if self.bias_k is not None:
            xavier_normal_(self.bias_k)
        if self.bias_v is not None:
            xavier_normal_(self.bias_v)
    ...
    Shape:
        ...
        if not self._qkv_same_embed_dim:
            ...
            if self.prompts_lora == -1:
                return multi_head_attention_prompts_forward(
                    query, key, value, self.embed_dim, self.num_heads, self.num_prompts,
                    self.in_proj_weight, self.in_proj_bias, self.in_prompts_proj_weight, self.in_prompts_proj_bias,
                    self.in_proj_weight_lora_A, self.in_proj_weight_lora_B, self.scaling,
                    self.bias_k, self.bias_v, self.add_zero_attn,
                    self.dropout, self.out_proj.weight, self.out_proj.bias, self.out_prompts_proj.weight, self.out_prompts_proj.bias,
                    self.out_proj.lora_A, self.out_proj.lora_B, self.out_proj.scaling,
                    training=self.training,
                    key_padding_mask=key_padding_mask, need_weights=need_weights,
                    attn_mask=attn_mask)
            else:
                return multi_head_attention_prompts_forward2(
                    query, key, value, self.embed_dim, self.num_heads, self.num_prompts,
                    self.in_proj_weight, self.in_proj_bias, self.in_prompts_proj_weight, self.in_prompts_proj_bias,
                    self.in_proj_weight_lora_A, self.in_proj_weight_lora_B, self.in_prompts_proj_weight_lora_A, self.in_prompts_proj_weight_lora_B, self.scaling,
                    self.bias_k, self.bias_v, self.add_zero_attn,
                    self.dropout, self.out_proj.weight, self.out_proj.bias, self.out_prompts_proj.weight, self.out_prompts_proj.bias,
                    self.out_proj.lora_A, self.out_proj.lora_B, self.out_prompts_proj.lora_A, self.out_prompts_proj.lora_B, self.out_proj.scaling,
                    training=self.training,
                    key_padding_mask=key_padding_mask, need_weights=need_weights,
                    attn_mask=attn_mask)
